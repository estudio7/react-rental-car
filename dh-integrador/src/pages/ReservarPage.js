...
const Reserva = ({ match }) => {
  ...
  useEffect(() => {
    ...
    const fetchUsuario = async () => {
      ...
      const response = await axios.get(`http://3.92.126.107:5000/usuario/${userId}`, {
    ...
    const fetchProduto = async () => {
      const response = await axios.get(`http://3.92.126.107:5000/produto/${idProduto}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://3.92.126.107:5000/reserva', data, {
        headers: { Authorization: `Bearer ${token}` },
      });
